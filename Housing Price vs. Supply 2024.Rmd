...
housing_price_hist <- ggplot(housing_2024_all, aes(x = Median_Sale_Price_per_k)) +
...
allstatescatterplot <- ggplot(housing_2024_all_plot, aes(x = Months_Supply, y = Median_Sale_Price_per_k)) +
...
state_medians <- housing_ridge %>%
  group_by(State) %>%
  summarize(
    Median_Sale_Price = median(Median_Sale_Price_per_k, na.rm = TRUE),
    .groups = "drop"
  )
