...
housing_2024 = housing %>% filter(housing$Year == 2024, 
                                  ...
                                  housing$County != 'AA State') %>% na.omit %>% #excluding the armed forces
...
#all
ggplot(housing_2024_all, aes(x = reorder(State, -Median_Sale_Price_in_k, median), 
...
ggplot(housing_compare_all, aes(x = State, y = Median_Sale_Price_in_k, fill = StateGroup)) +
  geom_boxplot() +
...
ggplot(housing_constr, aes(x = constr_bins, y = Median_Sale_Price_in_k,fill=constr_bins)) +
  geom_boxplot() +
  labs(
...
```{r supply option 1}
#housing supply dist w all states for context, all df
housing_median_supply <- housing_2024_all %>%
  group_by(State) %>%
  summarize(Median_Months_Supply = median(Months_Supply, na.rm = TRUE)) %>%
  arrange(desc(Median_Months_Supply))

ggplot(housing_median_supply, aes(x = reorder(State, Median_Months_Supply), y = Median_Months_Supply)) +
  geom_segment(aes(xend = State, y = 0, yend = Median_Months_Supply), color = "gray80") +
  geom_point(color = "steelblue", size = 3) +
  coord_flip() +
  labs(
    title = "Median Months' Supply by State (2024)",
    x = "State",
    y = "Median Months of Supply",
    caption = "SOURCE: American Enterprise Institute"
  ) +
  theme_minimal()
#this one is fine, would prob be better with just 12
```

```{r supply option 2}
#housing supply dist w all states for context, all df
ggplot(housing_2024_all, 
       aes(x = pmin(Months_Supply, 12), y = reorder(State, Months_Supply))) +
  geom_density_ridges(
    fill = "steelblue", alpha = 0.7, scale = 2
  ) +
  labs(
    title = "Distribution of Months' Supply by State (Capped at 12 Months)",
    subtitle = "Months' Supply values greater than 12 are capped for visualization purposes",
    x = "Months of Supply (Capped at 12)",
    y = "State",
    caption = "SOURCE: American Enterprise Institute"
  ) +
  theme_minimal()
#this is crazy to look at- don't plan to include
```
