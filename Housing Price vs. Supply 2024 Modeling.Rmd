...
This includes both percents as well as margins of error. I have included code to drop the margins of error.
...
```{r household rank density}
# ggplot(inventory_2024_filtered, aes(x = HouseholdRank)) +
#   geom_histogram(binwidth = 50, fill = "steelblue", color = "white", alpha = 0.7) +
#   labs(
#     title = "Distribution of HouseholdRank",
#     x = "HouseholdRank (Metro Size)",
#     y = "Count") +
#   theme_minimal()
```
...
ggplot(inventory_2024_filtered, aes(x = total_listing_count, y = log_median_price_sqft)) +
  geom_point(alpha = 0.6, color = "steelblue") +
  labs(
    title = "Log price vs Total Supply",
    x = "Total Listing Count",
    y = "Median Price per Square Foot") +
  theme_minimal()
```
...
ggplot(full_dataset_clean %>% filter(cbsa_title %in% top_metros),
       aes(x = cbsa_title, y = median_listing_price_per_square_foot)) +
...
#plot
ggplot(combined_metros, aes(x = cbsa_title, y = median_listing_price_per_square_foot, fill = listing_group)) +
  geom_boxplot() +
  scale_y_continuous(labels = scales::comma) +
...
reg_model_by_all <- lm(median_listing_price_per_square_foot ~ . -GEOID -cbsa_title,
            data = train)

summary(reg_model_by_all)
```

Have to eliminate GEOID and cbsa_title or we would overfit
...
```{r}
#reg d- "avg_log_median_listing_price" i- "total_active_listing_count" "area_type" "avg_pct_income_75k_plus" "avg_pct_units_built_2000_plus" "avg_pct_bachelors_or_higher" 
reg_model_log_by_all <- lm(log_median_price_sqft ~ . -GEOID -cbsa_title,
            data = train)
...
inventory_model_simple <- inventory_2024 %>%
  select(
    median_listing_price_per_square_foot,  #Target
    total_listing_count,                   #Supply
    pending_ratio,                         #Market Tightness
    median_days_on_market                  #Market Speed
  )
...
reg_tree_simple <- rpart(
  ...
  method = "anova",
  control = rpart.control(maxdepth = 3, minbucket = 200)  # controls complexity
)
...
summary(inventory_2024$total_listing_count)
# summary(inventory_2024$)
...
tree_preds_med <- predict(reg_tree_exp_med, tree_data_med)

pred_df_med <- tibble(actual = tree_data_med$median_listing_price_per_square_foot,
                      predicted = tree_preds_med)

ggplot(pred_df_med, aes(x = actual, y = predicted)) +
  geom_point(alpha = 0.5) +
  geom_abline(intercept = 0, 
              slope = 1, 
              linetype = "dashed") +
  labs(title = "Predicted vs Actual: Expanded Regression Tree",
       x = "Actual Median Price per Sq.Ft",
       y = "Predicted Median Price per Sq.Ft") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) +
  theme_minimal()

#stephs expanded regression tree (log)
tree_data_log <- train %>%
  mutate(across(where(is.character), 
                as.factor)) %>%
...
set.seed(42)
reg_tree_exp_log <- rpart(log_median_price_sqft ~ .,
                      data = tree_data_log,
                      method = "anova",
                      control = rpart.control(maxdepth = 4, 
                                              minbucket = 120, 
...
rpart.plot(reg_tree_exp_log,
           type = 2,
           extra = 101,
           under = TRUE,
           fallen.leaves = TRUE,
           roundint = FALSE,
           main = "Expanded Regression Tree: Log Price per Sq.Ft",
           box.palette = "Blues",
           tweak = 1.9)

tree_preds_log <- predict(reg_tree_exp_log, tree_data_log)

pred_df_log <- tibble(actual = tree_data_log$log_median_price_sqft,
                  predicted = tree_preds_log)

ggplot(pred_df_log, aes(x = actual, y = predicted)) +
  geom_point(alpha = 0.5) +
  geom_abline(intercept = 0, 
              slope = 1, 
              linetype = "dashed") +
  labs(title = "Predicted vs Actual: Expanded Regression Tree",
       x = "Actual Log Price per Sq.Ft",
       y = "Predicted Log Price per Sq.Ft") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) +
  theme_minimal()
```
...
```{r}
# remove rows with missing predictor/target values
inventory_model_simple <- inventory_model_simple %>% na.omit()
...
rf_model <- randomForest(
  ...
  mtry = 2,          # number of variables per split (sqrt rule works here)
  importance = TRUE,
  na.action = na.omit    # <--- FIX
)
...
# Scale Node Purity to thousands
varImp[,2] <- varImp[,2] / 1000

colnames(varImp)[2] <- "IncNodePurity (x1000)"

varImpPlot(rf_model,
           main = "Variable Importance (Market Drivers)",
           cex.main = 1.2, 
           cex.lab = 0.9, 
           cex.axis = 0.8)
varImpPlot(rf_model,
           main = "Importance of Supply & Demand Drivers",
           col = "steelblue",
           pch = 19,
           cex.main = 1.3,
           cex.lab = 1,
...
Modeling- regression forest (baylee) (ilgaz)
