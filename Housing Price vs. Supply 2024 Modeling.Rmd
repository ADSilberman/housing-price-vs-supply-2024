...
train = subset(full_dataset_clean, split == TRUE)
test = subset(full_dataset_clean, split == FALSE)
...
```{r lasso_price_best}
lasso_price_best_model = glmnet(X_train, price_train, alpha = 1, lambda=lasso_price_best_lambda)
...
#stephs expanded regression tree
tree_vars <- c("median_listing_price_per_square_foot",
               "total_listing_count",
               "pending_ratio",
               "median_days_on_market",
               "total_population",
...
tree_data <- full_dataset_clean %>%
  select(any_of(tree_vars)) %>%
  mutate(area_type = as.factor(area_type)) %>%
  na.omit()

set.seed(42)
reg_tree_exp <- rpart(median_listing_price_per_square_foot ~ .,
                      data = tree_data,
                      method = "anova",
...
rpart.plot(reg_tree_exp,
           type = 2,
           extra = 101,
           under = TRUE,
           fallen.leaves = TRUE,
           roundint = FALSE,
           main = "Expanded Regression Tree: Price per Sq.Ft",
           box.palette = "Blues",
           tweak = 2)

tree_preds <- predict(reg_tree_exp, tree_data)

pred_df <- tibble(actual = tree_data$median_listing_price_per_square_foot,
