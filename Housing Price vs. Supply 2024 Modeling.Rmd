...
In the first project, we examined the relationship between months of supply and median home prices across U.S. metro areas in 2024 using data from the American Enterprise Institute (AEI). 

While the analysis identified a clear association between months of supply and median home prices, further exploratory analysis revealed that months of supply is not a raw measure but rather reflects the pace of inventory absorption. This limited our ability to fully understand housing price dynamics and motivated us to seek out new datasets.

The second project uses raw supply measures while incorporating socioeconomic context. The analysis relies on the CBSA-level 2024 Realtor.com dataset as the core housing market dataset. This dataset is enriched with 2024 one-year estimates from the American Community Survey (ACS) to capture demographic, economic, and housing characteristics across U.S. metros. 

From the 2024 ACS 1-Year Estimates Data Profiles, we selected variables that provide an overview of population composition, income, and housing characteristics for each CBSA included in the Realtor.com data.

By integrating housing market indicators with structural socioeconomic variables, this project aims to better distinguish whether observed housing price patterns in 2024 are primarily driven by supply constraints, demand-side pressures, or underlying demographic and economic conditions across U.S. metropolitan areas.

## Data 

This project uses the CBSA-level 2024 Realtor.com dataset referenced as the source data from our Project 1 data source: the American Enterprise Institute. We enrich this dataset with the 1-year estimate data for 2024 from the American Community Survey (ACS). The variables are:  
...
From the 2024 ACS 1-Year Estimates Data Profiles (U.S. Census Bureau, 2024), demographic, economic, and housing variables are selected, giving a broad overview of the characteristics of each CBSA in the Realtor.com dataset. Additionally, where necessary, variables have been removed to eliminate multicollinearity.
...
inventory_raw <- read_csv("../Data/RDC_Inventory_Core_Metrics_Metro_History.csv")
...
The data is split, with 80% for training and 20% for testing. For the purposes of this study, both the median listing price per square foot and its log were used as target variables, so that they may be compared.

Matrices are also prepared for use by LASSO.
...
First, a linear model is fit with `total_listing_count` as the only predictor. With such a simple model, it is perhaps not surprising that the Multiple R-squared is so low—0.0288—though this is still highly significant. However, as the estimate of this coefficient is positive, this model would seem to imply that increased supply is correlated with increased price per square foot. Perhaps more information will be more instructive.
...
All CBSA title variables had to be eliminated, or there would be more columns than observations and the model would overfit. With the full set of housing and demographic controls included, overall fit improves substantially. Several measures tied to income, housing composition, and vacancy are statistically significant, suggesting that price differences across markets reflect broader economic and housing structure rather than supply alone. Notably, the coefficient on total listing count changes sign once these controls are included, which points to confounding in the simple bivariate relationship.

#### Log Median Listing Price per Square Foot
```{r}
...
Logging the dependent variable nearly doubles the Multiple R-squared relative to the unlogged specification, indicating a modest improvement in fit. This suggests the log transformation reduces skew and the influence of extreme values. Even then, the model still explains only a limited share of the variation in price per square foot.
...
The CBSA title variable is excluded to avoid overfitting. Using the log-transformed outcome produces a slightly better overall fit than the unlogged version, consistent with variance being more stable in log terms. Many income, housing stock, and demographic variables remain statistically significant, and the coefficient on total listing count is again negative once these controls are included, reinforcing that the simple supply–price relationship does not persist after accounting for broader market structure.
...
The LASSO model uses cross-validation to pick a penalty (0.0221) that balances fit and complexity. At that setting, many coefficients are shrunk exactly to zero (the “.” entries), so the model is doing built-in feature selection while keeping only the strongest signals. The remaining nonzero terms include a mix of socioeconomic variables and CBSA indicators, which suggests that a meaningful share of price differences is still place-specific even after accounting for observed market and demographic controls.

##### Median Listing Price per Square Foot Without CBSA Codes
...
This version of the LASSO excludes CBSA fixed effects and therefore focuses only on observable housing and demographic characteristics. The cross-validated penalty (0.0087) retains a relatively broad set of predictors, indicating that many structural features still contribute meaningfully to price variation even without place-specific controls. Compared to the CBSA-inclusive model, the coefficients are more interpretable as general market relationships, and the negative coefficient on total listing count aligns with the expectation that higher supply is associated with lower prices once broader conditions are accounted for.
...
For the log price LASSO, cross-validation selects a much smaller penalty (0.000093), which means the model shrinks less aggressively and keeps many CBSA indicators in play. Even so, several broad market factors still show up with non-zero coefficients, and total listing count remains slightly negative, consistent with a small downward association between supply and prices once other features are considered. Overall, this suggests the log scale is easier to fit with fine-grained location effects while still preserving a handful of general relationships outside of CBSA alone.

##### Log Median Listing Price per Square Foot Without CBSA Codes
...
```
For the log model without CBSA codes, cross-validation again picks a very small penalty ((λ ≈ )0.000093), so most of the remaining predictors stay in the model rather than being heavily shrunk to zero. With location effects removed, the LASSO leans more on broad market fundamentals: income brackets and housing stock variables retain non-zero coefficients, vacancy and owner-occupancy remain important, and total listing count is still slightly negative, suggesting a small inverse association with prices even after regularization.

### Regression Tree


We also estimated an expanded regression tree to capture nonlinear relationships between median price per square foot and socioeconomic and housing characteristics. The resulting tree makes one point very clear: income dominates every other predictor. The initial split occurs at median household income, separating a small group of high-income markets with substantially higher prices from the broader set of lower-income areas. Within the lower-income group, the model again splits on income, showing that income consistently outweighs other demographic factors in explaining price differences. Only after these income thresholds are established do additional variables add nuance. Overall, the regression tree suggests that price per square foot is primarily structured by income levels, with demographic and housing characteristics playing a secondary role.
...
imp_df %>%
  ...
  coord_flip() +
  labs(title="Random Forest for Median Price per sqft.",
       x="Feature",
       y="MSE-contribution (%)") +
...
The Random Forest plot remains the same after using log(median_price) because the log transformation rescales the target without changing the order and structure for data splitting. As a result, the forest variable importance rankings stay unchanged (see p.371 Hastie, Tibshirani & Friedman (2009), Elements of Statistical Learning ).
...
imp_log_df %>%
  ...
  geom_col(fill="steelblue") +
  coord_flip() +
  labs(title="Random Forest for log(Median Price) per sqft.",
       x="Feature",
       y="MSE-contribution (%)") +
  theme_minimal(base_size = 13)
```
...
rmse_df = data.frame(Model = c("reg_model_by_1", "reg_model_by_all", "lasso_price_best_model", "lasso_price_no_cbsa_best_model", 
                               #"reg_tree_simple", 
                               "reg_tree_exp_med", "rf_exp_med", "reg_model_log_by_1", "reg_model_log_by_all", "lasso_log_price_best_model", "lasso_log_price_no_cbsa_best_model", "rf_exp_log"),
                     RMSE = c(reg_model_by_1_rmse, reg_model_by_all_rmse, lasso_price_best_model_rmse, lasso_price_no_cbsa_best_model_rmse, 
                              #reg_tree_simple_rmse, 
                              reg_tree_exp_med_rmse, rf_exp_med_rmse, reg_model_log_by_1_rmse, reg_model_log_by_all_rmse, lasso_log_price_best_model_rmse, lasso_log_price_no_cbsa_best_model_rmse, rf_exp_log_rmse)
                     )
...
residual_reg_by_1 <- ggplot(data = data.frame(actual = price_test,
                                              residual = reg_model_by_1_pred - price_test),
                            aes(x = actual, 
                                y = residual)) +
  geom_point(alpha = 0.6, 
             color = "steelblue") +
  labs(title = "Residual Plot: Linear Model (Supply Only)",
       x = "Actual",
       y = "Residual",
       caption = "SOURCE: Realtor.com") +
  theme_minimal(base_size = 11) +
...
residual_lasso <- ggplot(data = data.frame(actual = price_test,
                                           residual = as.numeric(lasso_price_no_cbsa_best_model_pred) - price_test),
                         aes(x = actual, y = residual)) +
  geom_point(alpha = 0.6, 
             color = "steelblue") +
  labs(title = "Residual Plot: LASSO (No CBSA)",
       x = "Actual",
       y = "Residual",
       caption = "SOURCE: Realtor.com") +
  theme_minimal(base_size = 11) +
...
residual_tree <- ggplot(data = data.frame(actual = price_test,
                                          residual = reg_tree_exp_med_pred - price_test),
                        aes(x = actual, y = residual)) +
  geom_point(alpha = 0.6, 
             color = "steelblue") +
  labs(title = "Residual Plot: Regression Tree (Expanded)",
       x = "Actual",
       y = "Residual",
       caption = "SOURCE: Realtor.com") +
  theme_minimal(base_size = 11) +
...
residual_rf <- ggplot(data = data.frame(actual = price_test,
                                        residual = rf_exp_med_pred - price_test),
                      aes(x = actual, y = residual)) +
  geom_point(alpha = 0.6, 
             color = "steelblue") +
  labs(title = "Residual Plot: Random Forest (Median Price)",
       x = "Actual",
       y = "Residual",
       caption = "SOURCE: Realtor.com") +
  theme_minimal(base_size = 11) +
...
Overall, the analysis shows that housing prices across U.S. metropolitan areas are most strongly associated with two structural factors: the amount of housing inventory available and median household income. Across all modeling approaches we applied, these variables consistently appeared as the most influential predictors of median price per square foot. Measures of market dynamics, such as pending ratio and days on market, helped differentiate between types of high-price markets, particularly those characterized by faster turnover versus those driven by higher incomes, but they did not outweigh the importance of income and supply. While the agreement across models increases confidence in these relationships, the analysis relies on cross-sectional, metro-level data. Consequently, the results should be interpreted as descriptive associations rather than causal effects, and variation within individual metropolitan areas may not be fully captured. Even with these limitations, the consistency of findings across methods suggests that differences in income levels and housing supply remain central to understanding variation in housing prices across U.S. markets.

### Further Applications

For further applications, the modeling approach can be extended beyond Random Forest estimation to explore feature selection and model comparison. Feature engineering using Random Forest indicators can be performed by selecting the most important variables identified by the Random Forest and reducing dimensionality to build interpretable linear models. Additionally, applying XGBoost may improve predictive accuracy relative to the Random Forest.
